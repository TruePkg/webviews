/* eslint-disable complexity,max-statements,max-params,max-depth */

/**
 * This is a port over of the Confidence library making it workable
 * outside of a pure node environment
...
export default (internals.Store = function(document) {
  this.load(document || {})
})
...
internals.filter = function(node, criteria, applied) {
  ...
  if (node.$value) {
    return internals.defaults(
      internals.filter(node.$value, criteria, applied),
      node.$base
    )
  }
  ...
  if (criterion !== undefined) {
    if (node.$range) {
      ...
      return internals.defaults(
        internals.filter(node[criterion], criteria, applied),
        node.$base
      )
    }
  ...
  if (Object.prototype.hasOwnProperty.call(node, '$default')) {
    ...
    return internals.defaults(
      internals.filter(node.$default, criteria, applied),
      node.$base
    )
  }
...
export const _logApplied = function(applied, filter, node, criterion) {
  if (!applied) {
    return
  }
...
internals.Store.validate = function(node, path) {
      ...
      if (key === '$filter') {
        ...
        for (let j = 0; j < node.$range.length; ++j) {
          ...
          if (lastLimit !== undefined && range.limit <= lastLimit) {
            return error(
              `Range entries not sorted in ascending order - ${
                range.limit
              } cannot come after ${lastLimit}`
            )
          }
          ...
          const err = internals.Store.validate(
            range.value,
            `${path}/$range[${range.limit}]`
          )
          if (err) {
            return err
          }
  ...
  if (
    found.value &&
    (found.key || found.range || found.default || found.filter)
  ) {
